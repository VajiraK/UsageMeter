...
bool CMyLogFile::LoadLog()
{
	...
	ReadFile(m_logthread_data.hlogfile,m_logthread_data.data,m_logthread_data.len,&m_logthread_data.len,NULL);
...
HKEY				hKey;
DWORD				len;
char				workdir[MAX_PATH],apppath[MAX_PATH];

	ZeroMemory(workdir,MAX_PATH);
	...
	{//Cannot get application path
		WriteLog("Can not get application path");
		return;
	}
...
DWORD i;
	...
	len = i;
	memcpy(workdir,apppath,i+1);
	memcpy(&apppath[i],"\\Viewer.exe\0",12);
	len = len + 12;

	//Write working dir
	i++;
	RegSet(HKEY_LOCAL_MACHINE,"WorkingDir",workdir,i);
	...
	len = len + 3;
	i = RegSetValueEx(hKey,NULL,0,REG_SZ,(LPBYTE)apppath, len);
				RegCloseKey(hKey);

	if( i != ERROR_SUCCESS )
	{//Last func fail
		WriteLog("Cannot register Usage Meter log files");
	}
}
//-----------------------------------------------------------------------------------------------
