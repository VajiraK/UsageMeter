// Usage Meter.cpp : Defines the entry point for the application.
//************ 10-07-2004 18:49 ****************
...
try
{
	...
	Main_Window.m_mutex = CreateMutex(NULL,TRUE,ID_MUTEX_STR);
...
int wmId, wmEvent;
	...
	case WM_SYSCOMMAND://+++MSG+++
		...
		{
			case SC_MAXIMIZE:
				return 0;//MAXIMIZE button press
			break;
			case ID_MENU_PERF:
...
LRESULT CALLBACK PerformanceDlg(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message)
	{
		...
		char			output[15];
			...
			StringCbPrintf(output,15," Kernel - %i ms",d1);
			...
			StringCbPrintf(output,15,"Free - %i MB",memst.dwAvailPhys);
			...
			StringCbPrintf(output,15,"Tatal - %i MB",memst.dwTotalPhys);
...
LRESULT CALLBACK BeepDlg(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message)
	{
		...
		POINT spt,cpt;
			...
			{
				if(cpt.y<22)
					DialogBox(g_hInstance,(LPCTSTR)IDD_TIP_DLG,NULL,(DLGPROC)TipWindow);
			}
			break;
		case WM_DESTROY:
			break;
...
LRESULT CALLBACK TipWindow(HWND hTip, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message)
	{
		...
		break;
		case WM_LBUTTONDOWN:
			CreateWindow("STATIC","Text.BMP",SS_BITMAP|WS_VISIBLE|WS_CHILD,5,5,50,200,hTip,(HMENU)ID_BUT_OK,g_hInstance,NULL );
		break;
	}
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon			= LoadIcon(hInstance, (LPCTSTR)IDI_USAGEMETER);
...
DWORD	x,y;
	...
	hWnd = CreateWindow("MeterWindow","Usage Meter", WS_OVERLAPPEDWINDOW,250,100,515,552,GetDesktopWindow(),NULL,hInstance,NULL);
