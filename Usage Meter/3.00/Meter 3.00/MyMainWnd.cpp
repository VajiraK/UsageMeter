...
TCITEM ti;
	...
	//List views
	m_win_list.m_hlist = CreateWindow(WC_LISTVIEW,"MeterList",WS_BORDER|WS_CHILD|LVS_REPORT|LVS_SHOWSELALWAYS,2,2,485,441,hbut,NULL,m_hinstance,NULL); 
	m_int_list.m_hlist = CreateWindow(WC_LISTVIEW,"MeterList",WS_BORDER|WS_VISIBLE|WS_CHILD|LVS_REPORT|LVS_SHOWSELALWAYS,2,2,485,441,hbut,NULL,m_hinstance,NULL); 
...
DWORD contype;
	...
	KillTimer(m_hmain_window,TIMER_EXIT);
	//Save Window Pos
	SaveWindowPos();
	DeleteFonts();
...
NOTIFYICONDATA nd;
	...
	nd.hIcon = LoadIcon(m_hinstance, (LPCTSTR)IDI_USAGEMETER3);
	...
	if (Shell_NotifyIcon(Task, &nd)==0)
		WriteLog("Error in setting tray icon");
}
//--------------------------------------------------------------------------------------
void CMyMainWnd::StartUp()
{
	...
	if (m_privilage==CLIENT)
	{
		...
		SetTimer(m_hmain_window,TIMER_SERVER_LINK,500,0);
		//Write log
	...
	if(m_ex.err)
	{
		if(m_ex.kill)
		{
		/*	AddExeption("Usage Meter is terminating!");
			ShowExeptions();*/
			KillMeter();
			return;
		}
	...
	SetTimer(m_hmain_window,TIMER_EXIT,4000,0);
}
//--------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------
void CMyMainWnd::ServerTimerTicks()
{//Client call this
