...
bool	err = false;
	...
	for(int i=0;i<=1;i++)
	{
		...
		{//Interval
			err = true;
			strcpy_s(msg,50,"Miminum value for interval is 1min ");	
		}
...
char	temp[10];
	...
	{//Error in registry settings let's reset it
		m_beep_arr[0].frequency = 2000;
		m_beep_arr[0].duration	= 100;
		m_beep_arr[0].interval	= 60;
		m_beep_arr[0].enable	= 0;
		m_beep_arr[1].frequency = 1000;
